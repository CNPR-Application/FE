...
import { NotificationResponse, NotiPutRequest } from 'src/interfaces/Notification';
...
export class NotificationAdminDialogComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<NotificationAdminDialogComponent>,
  ...
  editNotification(noti: NotificationResponse) {
    ...
    if (noti.notificationId && noti.isRead == false) {
      this.api.editNotification(request, noti.notificationId).subscribe(
        (response) => {
          console.log('Edit noti ' + noti.notificationId + ' result: ' + response);
        },
        (error) => {
